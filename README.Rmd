...
![Figure 1. The workflow and usage process of the seperate functions. Each function creates an output which can be used for another function.](man/figures/workflow_visual.jpg)
...
<ul><li> 4 .tsv files containing the original SBtab's as provided by M. Teunis from University of Applied Sciences Utrecht.</li>
...
<li> 6 .rds files starting with merged_*. These were aquired using the merge_sbtab function. </li>
...
```{r sep_sbtab example, eval=FALSE}
...
```{r prep_merge example, eval=FALSE}
...
```{r merge_prepped example, eval=FALSE}
...
```{r merge_sbtab example, eval=FALSE}
