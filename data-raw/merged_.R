## code to prepare `merged_` files as found in folder data/merged

## merged_physmap67_*
merge_prepped(c("data/edges/prepped_physmap6_edges.rds", "data/edges/prepped_physmap7_edges.rds"),
              "merged_physmap67_edges", "data/merged")
merge_prepped(c("data/species/prepped_physmap6_species.rds", "data/species/prepped_physmap7_species.rds"),
              "merged_physmap67_species", "data/merged")
merge_prepped(c("data/compartments/prepped_physmap6_compartments.rds", "data/compartments/prepped_physmap7_compartments.rds"),
              "merged_physmap67_compartments", "data/merged")

## merged_physmap6789_*
merge_prepped(c("data/edges/prepped_physmap6_edges.rds", "data/edges/prepped_physmap7_edges.rds",
                "data/edges/prepped_physmap8_edges.rds", "data/edges/prepped_physmap9_edges.rds"),
              "merged_physmap6789_edges", "data/merged")
merge_prepped(c("data/species/prepped_physmap6_species.rds", "data/species/prepped_physmap7_species.rds",
                "data/species/prepped_physmap8_species.rds", "data/species/prepped_physmap9_species.rds"),
              "merged_physmap6789_species", "data/merged")
merge_prepped(c("data/compartments/prepped_physmap6_compartments.rds", "data/compartments/prepped_physmap7_compartments.rds",
                "data/compartments/prepped_physmap8_compartments.rds", "data/compartments/prepped_physmap9_compartments.rds"),
              "merged_physmap6789_compartments", "data/merged")
